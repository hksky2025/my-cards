    ...
    el.innerHTML = `
        <div class="progress-card">
            ...
            <div class="progress-sub">
                ${reached
                    ? '✅ 已達年度保費上限 $130,000'
                    : `⏳ 再簽 <strong>$${(CCB_CAP - ccbInsYear).toLocaleString()}</strong> 達年度上限（信用額 $${CCB_CAP.toLocaleString()}）`}
            </div>
        </div>
    `;
}
...
function renderCapProgress(cards, getCardTotal) {
    ...
    const CAP_INFO = {
        ...
        mmpower:[{ label: '三類共享額外回贈上限$500（指定商戶8%/網購5%/海外4%）', cap: Math.round(500/0.046) }],
