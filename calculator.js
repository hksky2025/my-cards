...
export function calcBaseReward(card, params) {
    ...
    if (cat === 'Insurance') {
        // 渣打、DBS、中信、恒生：保險唔計回贈
        if (['sc', 'dbs', 'citic', 'hangseng'].includes(card.bank)) {
            return { val: 0, miles: 0, rate: '不適用（保費唔計回贈）' };
        }
        ...
        // 建銀：0.4%（全年上限視信用額，系統無法自動計算）
        if (card.bank === 'ccb') {
            return { val: amt * 0.004, miles: 0, rate: `0.4%（⚠️需網上繳費，全年上限視信用額）` };
        }
        // 其他銀行：當一般消費計
    }
